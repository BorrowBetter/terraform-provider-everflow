...
// TestAdvertiserResource_SchemaImplementation catches framework-level schema
// bugs (e.g. an attribute accidentally marked both Required and Computed) at
// test time rather than at provider startup.
func TestAdvertiserResource_SchemaImplementation(t *testing.T) {
	t.Parallel()

	r := NewAdvertiserResource()
	var resp fwresource.SchemaResponse
	r.(*AdvertiserResource).Schema(context.Background(), fwresource.SchemaRequest{}, &resp)
	...
	s, ok := resp.Schema.GetAttributes()["network_advertiser_id"]
	if !ok {
		t.Fatalf("schema missing network_advertiser_id")
	}
	if _, ok := s.(fwschema.Int64Attribute); !ok {
		t.Errorf("network_advertiser_id is %T, want schema.Int64Attribute", s)
	}
	if !s.IsComputed() {
		t.Errorf("network_advertiser_id must be Computed")
	}
...
// TestAdvertiserResource_CreateReadUpdateDelete exercises the full CRUD
...
//     network_advertiser_id + other computed fields
...
func TestAdvertiserResource_CreateReadUpdateDelete(t *testing.T) {
	t.Parallel()
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("everflow_advertiser.test", "name", "Acme"),
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("everflow_advertiser.test", "name", "Acme Renamed"),
					resource.TestCheckResourceAttr("everflow_advertiser.test", "internal_notes", "Managed by Terraform"),
...
// TestAdvertiserResource_ImportByID exercises the ImportState path: a bare
// numeric ID on the CLI must be parsed to int64 and land in
// network_advertiser_id, and the subsequent Read must hydrate the rest of
// the attributes.
func TestAdvertiserResource_ImportByID(t *testing.T) {
	t.Parallel()

	srv, _ := newAdvertiserTestServer(t, &advertiserRecord{
		ID:                  2,
		NetworkID:           1,
		Name:                "BorrowBetter",
		AccountStatus:       "active",
	...
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testProviderConfig(srv) + `
resource "everflow_advertiser" "imported" {
  name                  = "BorrowBetter"
  account_status        = "active"
  network_employee_id   = 11
  default_currency_id   = "USD"
...
`,
			...
			{
				ResourceName:                         "everflow_advertiser.imported",
				ImportState:                          true,
				ImportStateId:                        "2",
				ImportStateVerify:                    true,
				ImportStateVerifyIdentifierAttribute: "network_advertiser_id",
			},
		},
	})
}

// TestAdvertiserResource_ImportInvalidID verifies the string→int64 parser
// surfaces a clean diagnostic rather than silently dropping the value.
func TestAdvertiserResource_ImportInvalidID(t *testing.T) {
	t.Parallel()
...
`,
			...
			{
				ResourceName:  "everflow_advertiser.test",
				ImportState:   true,
				ImportStateId: "not-a-number",
				ExpectError:   regexp.MustCompile(`network_advertiser_id must be a base-10 integer`),
			},
		},
	})
}

// TestAdvertiserResource_Read404RemovesFromState simulates the "deleted
...
func TestAdvertiserResource_Read404RemovesFromState(t *testing.T) {
	t.Parallel()
...
// TestAdvertiserResource_InvalidAccountStatus verifies the
// stringvalidator.OneOf attached to account_status rejects values outside
// the allowed set at plan time (no HTTP call is made).
func TestAdvertiserResource_InvalidAccountStatus(t *testing.T) {
	t.Parallel()
...
// advertiserRecord is the in-memory fake Everflow's view of a single
// advertiser. Extra holds any nested objects (billing, contact_address,
// settings, ...) the typed schema does not model so the fake can
...
type advertiserRecord struct {
	...
	ReportingTimezoneID int64
	InternalNotes       string
	Extra               map[string]any
}

// advertiserServerState is the side-channel the tests use to inspect what
...
// newAdvertiserTestServer spins up a minimal in-memory fake Everflow that
// speaks the subset of the API the advertiser resource exercises:
//
//   - POST /v1/networks/advertisers — creates, returns the record with an
//     auto-assigned ID
//   - GET /v1/networks/advertisers/{id} — returns the record, honoring the
//     force404 override used by the out-of-band delete test
//   - PUT /v1/networks/advertisers/{id} — fully replaces the record from
//     the request body (Everflow's PUT semantics) and captures the raw
//     request body for the test to assert against
//   - DELETE /v1/networks/advertisers/{id} — flips deleteCalled=true so the
...
func newAdvertiserTestServer(t *testing.T, seed *advertiserRecord) (*httptest.Server, *advertiserServerState) {
	t.Helper()
	state := &advertiserServerState{record: seed}

	mux := http.NewServeMux()
	mux.HandleFunc("/v1/networks/advertisers", func(w http.ResponseWriter, r *http.Request) {
		state.mu.Lock()
		defer state.mu.Unlock()
		...
		case http.MethodPost:
			...
			state.record.ReportingTimezoneID = int64FromMap(body, "reporting_timezone_id")
			state.record.InternalNotes = stringFromMap(body, "internal_notes")
			writeAdvertiserRecord(w, state.record)
		default:
	...
	mux.HandleFunc("/v1/networks/advertisers/", func(w http.ResponseWriter, r *http.Request) {
		...
		// /v1/networks/advertisers/{id} — we only serve one record per
		...
		case http.MethodPut:
			...
			// nested objects (e.g. billing). The resource's fetch-modify-
			// put strategy should be echoing them back.
			state.record.Extra = map[string]any{}
			for k, v := range body {
				switch k {
				case "network_advertiser_id", "network_id", "name", "account_status",
					"network_employee_id", "default_currency_id", "reporting_timezone_id",
...
// writeAdvertiserRecord serializes the in-memory record back out as JSON in
// the same shape Everflow's real API uses. Extra fields are merged into the
// top-level object.
func writeAdvertiserRecord(w http.ResponseWriter, rec *advertiserRecord) {
	out := map[string]any{
		"network_advertiser_id": rec.ID,
		...
		"reporting_timezone_id": rec.ReportingTimezoneID,
...
// stringFromMap / int64FromMap are small helpers that keep the server
// handlers readable. Missing keys yield the zero value, which is fine for
// test assertions.
